...
export default function SettingsPage() {
  ...
  const [email, setEmail] = useState("raj.patel@corp.com");
      ...
      <div className="mx-auto flex max-w-xl flex-col gap-6">
        ...
        <Card className="p-6">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-amber-400 font-display text-xl font-bold text-white">
              {name.split(" ").map((n) => n[0]).join("")}
            </div>
            <div className="flex-1">
              {!editing ? (
                <>
                  <p className="font-display text-lg font-bold text-ink-800 dark:text-white">{name}</p>
