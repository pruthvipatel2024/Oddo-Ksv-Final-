...
export class VehiclesController {
  ...
  @ApiOperation({ summary: 'Register a new vehicle', description: 'Registers a vehicle to support publishing rides.' })
  ...
  @Get()
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'List all vehicles', description: 'Employees retrieve their own vehicles. Admins get records scoped to their bounds.' })
  ...
  async findAll(@CurrentUser() user: JwtPayload) {
    ...
    if (user.role === UserRole.ORGANIZATION_ADMIN) {
      return {
        success: true,
        data: await this.vehiclesService.findAll(user.organizationId || undefined),
      };
    }
  ...
  @Get(':id')
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Get vehicle by ID', description: 'Access is limited to the vehicle owner or Admins.' })
  ...
  @Roles(UserRole.EMPLOYEE)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Update vehicle details', description: 'Allows owner to update color, seating capacity, or photo.' })
  ...
  @Roles(UserRole.EMPLOYEE)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Delete a registered vehicle', description: 'Soft deletes the vehicle record.' })
  ...
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Verify vehicle registration status', description: 'Allows Admins to verify, suspend, or reject registered vehicles.' })
  ...
  async verify(
    ...
    return {
      ...
      data: await this.vehiclesService.verify(
        id,
        dto.status,
        admin.role === UserRole.SUPER_ADMIN ? undefined : admin.organizationId || undefined,
      ),
    };
  }
}
